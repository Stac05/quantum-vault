...
class AdaptiveChunkProcessor:
    ...
    def iter_chunks(self, source_path: PathLike, chunk_size: int | None = None) -> Iterator[bytes]:
        """Yield the source file in adaptive-size chunks.

        Args:
            source_path: The source file path.
            chunk_size: Optional fixed chunk size to use.
    ...
    def encrypt(
        ...
        with destination_file.open("wb") as out_file:
            for index, chunk in enumerate(self.iter_chunks(source_file, chunk_size)):
